---
name: dev-scenario-authoring
description: >
  Authoring reference for BlastSimulator2026 scenario definitions: step roles and what each may
  execute, step goals (`expect`), expected command outcomes (`commandOutcome`), step repetition,
  and the full interaction-action vocabulary including the wait-on-a-condition rule. Use when
  writing or editing a scenario JSON file under scripts/scenario-defs/, or when changing the
  runners and shared types that read them.
---

# Scenario Authoring

One scenario file feeds two verification channels: `scenario` (command mode, pure Node.js) and `visual` (interaction mode, real Puppeteer clicks). In command mode, a step whose command the console refuses (`success: false`) fails the scenario unless the step declares `commandOutcome`. The scenario-definitions rule states the invariants a scenario file must hold; this page is the field-by-field reference behind them.
...
`increased`/`decreased`/`equals`/`changedBy` are checked in **command mode** (`checkGoalAgainstState`, `scripts/shared/scenario-goal.ts`, wired into `runSteps`), unscoped — that evaluator is the one place every goal a scenario declares is actually asserted. `usable`/`blocked`/`tutorialStep` need a live page and check only in interaction mode, reusing `interaction-driver.ts`'s own `checkGoal` — command mode has no DOM. A scenario with no `expect` anywhere only proves the sequence ran, not that it did anything.

**Interaction mode re-checks the state goals too, minus the trajectory-coupled ones.** `scopeGoalToInteraction` (`scripts/shared/interaction-goal-scope.ts`) drops `equals`/`changedBy` on `tickCount` and `equals` on `cash` before handing the goal to `checkGoal`, and the runner reports each one it left behind rather than skipping it silently. This is the tick-divergence mechanism below, closed at the assertion rather than worked around per scenario: a `role: 'player'` step runs its whole `interaction` array while command mode runs one `command` string, so interaction mode legitimately spends ticks command mode does not, and an absolute `tickCount` — or a balance downstream of the event draws a diverged clock produces — is testing which trajectory the run took, not what the step did. Nothing leaves the suite: command mode asserts all of them on every pull request, in a fraction of the time and deterministically.

So write a browser claim as a browser claim. `usable`/`blocked`/`textEquals` prove a control's state; a step-local `changedBy` proves what this step's own actions cost (`changedBy: {cash: -1000}` stays checked in both modes — only the chained `equals: {cash: 48500}` is command mode's). Reaching for an absolute the run had to arrive at by one exact path is what `tests/unit/lint/ScenarioInteractionGoalsStayMeaningful.test.ts` bounds: a step whose whole `expect` is trajectory-coupled proves nothing in interaction mode beyond the clicks landing, which is a legitimate but capped resting state. `TRAJECTORY_COUPLED_GOAL_FIELDS` is an individually-reasoned list, not a pattern — a shard red on any other field is a finding, never a new entry.
...
| `setStepper` | Drive a `-/value/+` stepper (`spacing`, `depth`, `amount`, `stemming`, `delay-step`) to an exact `value`: `selector` names the `[data-field="…"]` container, the executor reads `.bsx-stepper-value`, clicks `+`/`-` through `clickSelector`'s usability gate until it matches, and fails by name if the control clamps first. The only way a scenario may move a stepper — a run of `.bsx-stepper-btn` clicks encoded the value as "default plus count" (PR #1070's first red shard) and is refused by `ScenarioStepperValueMatchesCommand.test.ts`, which also pins the value to the step's declared `spacing:`/`amount:`/… parameter. |
