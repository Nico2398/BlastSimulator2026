...
- **The loop budget.** `.claude/hooks/report-loop-budget.sh`, a `PreToolUse` hook on delegation in `.claude/settings.json`, tells the orchestrator before every delegation how much of `AGENTIC_LOOP_DEADLINE_EPOCH` is left, or that it has passed. `PreToolUse` is the event that honours `additionalContext`; a hook after the delegation returns cannot speak to the model. Without the variable the hook says nothing and the budget is open. OpenCode receives the same variable in its runner environment and nothing reports it: an OpenCode session reads the clock only if it does so itself under the shared rule — a gap recorded here, not closed.
