...
- **Tick loop steps:** (1) advance time, (2) weather, (3) events, (4) dispatch + locomotion + tasks, (5) physics (blast only), (6) scores, (7) win/lose check, (8) emit state-change events. The pipeline is core-owned, under `src/core/engine/`; `src/console/` and the browser render loop both call it rather than hosting their own ordering. Renderer runs at 60fps, interpolates between ticks.
- **One mobile agent, one journey object:** an employee is the only entity that moves. A vehicle is a tool they ride — its position is written from its occupant, never from a path of its own. Every journey is an `Itinerary`: an ordered list of foot/drive legs, each with an arrival step (board, alight, work effect), produced by `planItinerary` and consumed by the locomotion tick. `moveTo` is the only entry point that starts movement. Details and invariants: `gameplay-vehicle-fleet`.
- **One journey, one cost:** an action's queue cost is the itinerary's own cost, summed leg by leg. The estimator and the executor share `planItinerary` rather than measuring the same trip twice — a route calculation written outside it will drift from what the simulation does.
- **Arrival-gated position-dependent actions:** any action requiring an entity to physically be somewhere (survey, rest, boarding, hauling, digging) starts its timer or effect on the arrival of the leg that carries it there, never at claim time. Follow this pattern for any new position-dependent action.
...
- **Asset replaceability:** `src/renderer/models/ModelLibrary.ts` maps model ids (`ModelIds.ts`, derived from the core catalogs) → prototypes parsed from `public/models/*.glb`, generated by Blender from `assets/models/blender/` (`npm run models:build`). Mesh factories (`CharacterMesh`, `VehicleMesh`, `BuildingMesh`) instantiate from the library and recolour `Tint*` materials; an asset that has not loaded renders as a stand-in box until it does. Replace a model by regenerating its `.glb`; the runtime never changes.
...
│   ├── state/              # GameState, SaveLoad, SaveBackend interface, WorldInvariants
│   ├── engine/             # TickPipeline (tick orchestration), Itinerary + planItinerary (journey planning), Locomotion (the only mover), Mount (board/alight), TaskDispatch, ActionSelection (cost-based per-employee action ranking)
