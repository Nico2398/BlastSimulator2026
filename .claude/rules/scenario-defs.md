...
**2026-08-09 — target state, not current state.** THE MANDATE (`docs/plans/scenario-assertions-and-playtest-removal.md`) requires every non-cheat console command to have a real UI equivalent, and this suite — `role` plus `expect` together — to functionally replace `npm run playtest` (`scripts/playtest.ts`, `scripts/playtests/*.json`; documented in `.claude/rules/playability.md` and the `dev-playability-testing` skill), not merely duplicate it. Issue #515 supplies the structural piece: `role` is no longer a convention a step can skip but a lint (`tests/unit/lint/ScenarioStepsHaveRole.test.ts`), enforcing that every player-facing step is provably `player`/`setup`/`observe`/`bootstrap`/`guard`. That closes the tagging gap, but not the rest of the plan doc's Phase 3 — until it executes, playtest still exists, is still required for player-facing changes, and this file does not yet supersede it — do not delete playtest or its rule/skill on the strength of this section.
