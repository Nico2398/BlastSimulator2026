...
## Playtest replaced (issue #515)

**2026-08-09 — done, not aspirational.** THE MANDATE (`docs/plans/scenario-assertions-and-playtest-removal.md`) required every non-cheat console command to have a real UI equivalent, and this suite — `role` plus `expect` together — to functionally replace the old `npm run playtest` harness, not merely duplicate it. `role` is no longer a convention a step can skip but a lint (`tests/unit/lint/ScenarioStepsHaveRole.test.ts`), enforcing that every player-facing step is provably `player`/`setup`/`observe`/`bootstrap`/`guard`. The playtest script, its JSON definitions, and its own utility module are deleted; the former playability rule and skill are gone too, folded into this file and `dev-visual-testing`/`dev-testing-strategy`. `scripts/shared/playtest-types.ts` and `scripts/shared/playtest-driver.ts` survive — they are the shared foundation `interaction-executor.ts` and both interaction-mode runners depend on, not playtest's own code.
