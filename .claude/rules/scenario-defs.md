...
**Prefer `increased`/`decreased` over a chained `equals: {"cash": N}` for a step whose only assertion is an incidental running balance.** Reserve the exact figure for a step where the number itself is the point — an affordability refusal, a final win/lose threshold, a checkpoint the scenario's own conclusion depends on. A chained absolute couples every later step to everything before it: one upstream change (a new purchase inserted earlier, a duration that moved) forces every figure after it to be recomputed by hand across the whole file. This is most of what made #553's own scenario migration expensive — 51 files, most of the diff pure cash renumbering. A directional check on the same step still fails exactly when that step stops costing money or costs in the wrong direction, which is what the step is actually testing; it is not a weaker check, just one that stays true under changes that have nothing to do with it. Never make a scenario's own subject directional — deathCount, ecology/safety/wellBeing/nuisance scores, `tickCount` at a shutdown, star rating, `activeContractCount`, `levelEnded`/`levelEndReason` stay `equals`, always re-verified against the real numbers, never loosened to "moved in the right direction."
...
A wait for queued work to land (a drilled hole, and by issue #554 onward a loaded charge, a dug ramp segment, a built building) is today a hand-measured `tick N` step, console-verified per file — see `tutorial-playthrough.json`/`blast-basic.json` for the pattern. Issue #590 tracks a `waitUntil`-style primitive that asserts the field it's waiting on instead of just hoping the count still holds; until it lands, measure the number for real rather than copying one from another file — two files rarely share a crew, a pattern size, and a starting tick at once.
