...
78. **Issue #515 item 5 — playtest deleted.** With every scenario step's `role` structurally enforced (`tests/unit/lint/ScenarioStepsHaveRole.test.ts`, 0 untagged) and the content-parity table reconfirmed 5/5 closed (all five playtest definitions — `research-center-gate.json`, `scene-picking.json`, `training.json`, `tutorial-fr.json`, `tutorial.json` — have a scenario-suite equivalent that already covers what they proved), Phase 3's deletion executed: the playtest script itself, its whole JSON definitions directory, its own utility module, its dedicated unit test suite, the `playtest` npm script, and the `Playtest (playability)` CI job are all gone. `scripts/shared/playtest-types.ts` and `scripts/shared/playtest-driver.ts` survive unrenamed — they are `interaction-executor.ts`'s and both interaction-mode runners' shared foundation, not playtest's own code (the plan's original step 2, renaming them, was not done — no session this branch surfaced a reason the `playtest-*` filename itself was actually load-bearing enough to force a rename in the same change as the deletion; left as a possible future cleanup, not a gap in item 5's own scope). Two direct-coverage unit tests for `isAllowedSetupCommand`/`SETUP_COMMAND_ALLOWLIST` (`playtest-types.ts`'s one surviving non-comment consumer besides `interaction-executor.ts` itself) moved from the deleted playtest test suite into `tests/unit/playtest-types.test.ts` rather than being dropped. The former playability rule file and the `dev-playability-testing` skill (all three trees) are deleted; their still-true content folded into `.claude/rules/scenario-defs.md` (setup-role/allowlist notes, the unreachable-gate invariant), `.claude/rules/rendering.md` (`TileSelectOverlay` scoping, tutorial-advances-on-completion), and `dev-visual-testing` (the `__uiActions`/`__probeSelector`/`__tutorialState` bridges and the step-by-step unclickable-control diagnosis order). `.claude/CLAUDE.md`'s Verification Gate table dropped the `playability` row, folding its "no console command stands in for a player action" language into `visual`'s own row — mirrored to `.github/copilot-instructions.md` and `.opencode/AGENTS.md` (verified in each runtime's own voice, not copy-pasted verbatim, since these are entry points). `dev-testing-strategy`'s channel table and layer count dropped to four. `agentic-pipeline-pr-management` updated: `full-ci` now starts one browser job, not two (~30 min, not ~50), and its "playtest definition drives the change" test became "an interaction-mode scenario drives the change." `scripts/verify-env.ts` (live preflight code, not docs) dropped the `playability` channel entry and folded its `proves` text into `visual`'s. Verified: `npm run typecheck`, `npx vitest run`, `npm run scenarios` (command mode), `npm run validate:context` all green — see the PR for exact counts.
