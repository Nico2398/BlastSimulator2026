...
5. **Testing hooks are API:** stable ids/classes/data attributes are load-bearing (`#bs-toolbar [data-panel=…]`, `data-action`, `#bs-tile-select-confirm`, `.bs-detail-toggle`, `.bs-train-btn`, `data-employee-id`, …). The UI action probe (`__uiActions`, `__probeSelector`) and tutorial rails target them. The redesign may rename them, but then every tutorial `highlightTarget` (`src/ui/tutorialSteps.ts`), stage selector (`src/ui/tutorialStages.ts`), and scenario def (`scripts/scenario-defs/*.json`) must be updated in the same change. Prefer keeping ids.
