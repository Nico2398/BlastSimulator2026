...
describe('collectChangedLines', () => {
  it('collects the head-side lines of a multi-line hunk', () => {
    const changed = collectChangedLines(diff('src/a.ts', '@@ -10,0 +11,3 @@', '+one', '+two', '+three'));
    expect([...changed.get('src/a.ts')!.lines]).toEqual([11, 12, 13]);
  });

  it('treats a hunk with no count as a single line', () => {
    const changed = collectChangedLines(diff('src/a.ts', '@@ -4 +4 @@', '-old', '+new'));
    expect([...changed.get('src/a.ts')!.lines]).toEqual([4]);
  });

  it('accumulates every hunk in one file', () => {
    const changed = collectChangedLines(
      diff('src/a.ts', '@@ -1,0 +2,2 @@', '+a', '+b', '@@ -20,0 +30,1 @@', '+c'),
    );
    expect([...changed.get('src/a.ts')!.lines].sort((x, y) => x - y)).toEqual([2, 3, 30]);
  });

  it('keeps files apart', () => {
    const changed = collectChangedLines(
      [diff('src/a.ts', '@@ -0,0 +1,1 @@', '+a'), diff('scripts/b.ts', '@@ -0,0 +5,2 @@', '+b', '+c')].join('\n'),
    );
    expect([...changed.get('src/a.ts')!.lines]).toEqual([1]);
    expect([...changed.get('scripts/b.ts')!.lines]).toEqual([5, 6]);
  });
  ...
  it.each([
    ['tests/unit/a.test.ts', 'a test outside the scanned scope'],
    ['src/a.test.ts', 'a test file inside src/'],
    ['src/core/i18n/locales/en.json', 'a non-TypeScript file'],
    ['docs/notes.md', 'a file in an unscanned directory'],
  ])('ignores %s (%s)', (file) => {
  ...
  it('drops a file whose hunks added nothing on the head side', () => {
    ...
    const changed = collectChangedLines(diff('src/a.ts', '@@ -3,2 +2,0 @@', '-gone', '-also gone'));
...
describe('attribute', () => {
  ...
  it('reports zero when no clone touches a changed line', () => {
    const report = attribute(
      changed({ 'src/a.ts': [1, 2, 3] }),
      [clone('src/a.ts', 50, 60, 'src/b.ts', 10, 20)],
    );
  ...
  it('counts only the changed lines a clone actually covers', () => {
    ...
    const report = attribute(
      changed({ 'src/a.ts': [10, 11, 12, 13, 14] }),
      [clone('src/a.ts', 8, 12, 'src/b.ts', 1, 5)],
    );
    expect(report.duplicatedLines).toBe(3);
    expect(report.percentage).toBe(60);
    expect(report.hits).toEqual([{ file: 'src/a.ts', lines: 3, against: ['src/b.ts:1-5'] }]);
  });
  ...
  it('attributes a clone whichever side the changed file is on', () => {
    const report = attribute(
      changed({ 'src/new.ts': [1, 2, 3, 4, 5] }),
      [clone('src/untouched.ts', 40, 44, 'src/new.ts', 1, 5)],
    );
    expect(report.duplicatedLines).toBe(5);
    expect(report.hits[0]!.against).toEqual(['src/untouched.ts:40-44']);
  });

  it('counts a line inside two clones once, and names both counterparts', () => {
    const report = attribute(
      changed({ 'src/a.ts': [1, 2, 3] }),
      [clone('src/a.ts', 1, 3, 'src/b.ts', 7, 9), clone('src/a.ts', 2, 3, 'src/c.ts', 4, 5)],
    );
    expect(report.duplicatedLines).toBe(3);
    expect(report.hits[0]!.against).toEqual(['src/b.ts:7-9', 'src/c.ts:4-5']);
  });

  it('totals across files and ranks the worst offender first', () => {
    const report = attribute(
      changed({ 'src/a.ts': [1, 2], 'src/b.ts': [1, 2, 3, 4, 5, 6] }),
      [clone('src/a.ts', 1, 1, 'src/x.ts', 1, 1), clone('src/b.ts', 1, 4, 'src/y.ts', 1, 4)],
    );
    expect(report).toMatchObject({ changedFiles: 2, changedLines: 8, duplicatedLines: 5 });
    expect(report.percentage).toBe(62.5);
    expect(report.hits.map(h => h.file)).toEqual(['src/b.ts', 'src/a.ts']);
  });

  it('ignores a clone between two files the branch never touched', () => {
    const report = attribute(changed({ 'src/a.ts': [1] }), [clone('src/x.ts', 1, 9, 'src/y.ts', 1, 9)]);
    expect(report.duplicatedLines).toBe(0);
  });

  it('reports 0% rather than dividing by zero when nothing changed', () => {
    expect(attribute(changed({}), [clone('src/x.ts', 1, 9, 'src/y.ts', 1, 9)])).toMatchObject({
      changedLines: 0,
      percentage: 0,
    });
  });
  ...
  it('rounds the percentage to two decimals', () => {
    ...
    const report = attribute(changed({ 'src/a.ts': [1, 2, 3] }), [clone('src/a.ts', 1, 1, 'src/b.ts', 1, 1)]);
