...
10. **No label decides which CI jobs run.** The interaction-mode browser shards and the production build run on every pull request; `full-ci` and `build-check` no longer exist. They were opt-in with a "backend-only" exemption, and the exemption was wrong: PR #1068 changed one threshold in `src/core/` and broke three interaction scenarios, because the browser path depends on timing and most core changes move timing. An issue still names, in its Verification section, which scenario drives the change — that is what the reviewer and the `visual` channel read — but it no longer has to decide what CI proves.
...
13. **Label transfer.** A lifecycle label the PR needs at birth (`paused`, for a handover) is passed on the same `gh pr create` call that opens it — never a follow-up `gh pr edit --add-label`, which raises no `pull_request` event of its own. That rule was learned when PR #615 merged with its interaction-mode job silently skipped behind a label applied late; the job is unconditional now, the rule about opening-call labels stands. See `agentic-pipeline-finalization`'s `open-pr` step.
