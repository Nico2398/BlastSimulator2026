...
- **The matcher is half the guard, and it is the half that has failed.** Gating on `run_in_background` alone covers only the tools that *have* the parameter. `SendMessage` resumes an already-running sub-agent and `Monitor` waits for a condition — both are background by construction, neither takes the parameter, and a matcher of `Agent|Task` never shows the guard either one. PR #481 died in that gap: the orchestrator's `Agent` call was foreground and returned inside its turn, it continued the same agent with `SendMessage`, opened a `Monitor` to wait, and ended the turn on "I'll wait for the monitor's notification" — `num_turns: 0`, a half-finished rebase on the runner's disk, nothing pushed, and no hook event in the whole job log. The matcher is now `Agent|Task|SendMessage|Monitor`, the guard refuses the two resume tools outright rather than advising a foreground spelling that does not exist, and `validate:context` pins every name in `SETTINGS_HOOKS`. `Bash` stays deliberately unmatched: a backgrounded shell is how the visual and playability channels start `npm run dev`, and the run-killer was never the background process — it was ending the turn to wait on one.
