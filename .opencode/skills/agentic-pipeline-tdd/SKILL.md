...
```
  1. [setup-test-branch]     → (non-agentic) create pipeline/tests-<label> from <base_branch> (default: main)
  ...
  4. [verify-skeleton-sha]   → (non-agentic) confirm skeleton_commit_sha resolves: `git cat-file -t <sha>`. If invalid → retry @skeleton-writer.
  5. [verify-commit]         → (non-agentic) confirm skeleton commit exists; auto-commit if dirty
  6. [setup-impl-branch]     → (non-agentic) create pipeline/impl-<label> from skeleton_commit_sha
  7. [switch-to-test]        → (non-agentic) switch back to pipeline/tests-<label>
  8. [branch-sanity]         → (non-agentic) verify on pipeline/tests-<label>
 ...
 11. [switch-to-impl]        → (non-agentic) switch to pipeline/impl-<label>
 12. [branch-sanity]         → (non-agentic) verify on pipeline/impl-<label>
 ...
 15. [cherry-pick]           → (non-agentic) cherry-pick impl branch HEAD onto feature branch:
 ...
 16. [git-verify]            → (non-agentic) confirm clean state: git status, branch, last commits
```
