...
AGENTS.md, agent definitions, rules, and skills all use this convention. Two kinds of content. Obey their rules:
...
`.opencode/rules/` carries the hard invariants for one area of the tree, loaded on every session through `instructions` in `opencode.json`. Each one names the skill holding the detail. They mirror `.claude/rules/` and `.github/instructions/` word-for-word — edit all three together.
...
Carried by the `communication-style` rule (`.opencode/rules/communication-style.md`), loaded on every session through this project's `instructions` config. It is stated there once rather than restated here, so the three runtimes cannot drift apart on it.
