...
import { getDominantRockId, VOXEL_CELL_SIZE, type VoxelGrid, type VoxelData } from '../world/VoxelGrid.js';
    ...
    for (let y = bbox.minY; y <= bbox.maxY; y++) {
      for (let x = bbox.minX; x <= bbox.maxX; x++) {
        ...
        if (frag.result === 'fractured') {
          const voxelVolume = VOXEL_CELL_SIZE * VOXEL_CELL_SIZE * VOXEL_CELL_SIZE;
          ...
          totalOreValue += calculateOreValue(voxel, VOXEL_CELL_SIZE);
