...
/**
 * Lifecycle status of a PendingAction: 'queued' (unclaimed, waiting for a
 * qualified employee), 'assigned' (claimed by an employee who is still
 * walking to the target), 'in_progress' (employee has arrived and is working
 * the task). The record — and its ghost — is removed only on completion (#547).
 */
...
export interface PendingAction {
  ...
  /** Employee who claimed this action, or null while still 'queued' (#547). */
