// BlastSimulator2026 — Icon set (redesign P0)
//
// Ported 1:1 from docs/BlastSim game UI design/icons.js. Chunky filled
// geometric glyphs, 24px grid, currentColor. No emoji anywhere — every
// icon the toolbar/status/weather/controls need is drawn.
// Usage: iconEl('blast', 18) or <bs-icon name="blast" size="18"></bs-icon>
...
// Aliases used across the design docs.
PATHS['explosive'] = PATHS['blast']!;
PATHS['report'] = PATHS['contract']!;
PATHS['crane'] = PATHS['build']!;
PATHS['money'] = PATHS['finance']!;

export type IconName = keyof typeof PATHS | string;

class BsIcon extends HTMLElement {
  static get observedAttributes(): string[] { return ['name', 'size', 'op']; }
  ...
  private render(): void {
    ...
    const size = parseFloat(this.getAttribute('size') ?? '16');
    const op = this.getAttribute('op');
    if (!this.shadowRoot) this.attachShadow({ mode: 'open' });
    const root = this.shadowRoot!;
    root.innerHTML =
      `<style>:host{display:inline-flex;align-items:center;justify-content:center;` +
      `width:${size}px;height:${size}px;flex:0 0 auto;line-height:0` +
      `${op ? `;opacity:${op}` : ''}}svg{display:block;overflow:visible}</style>` +
      `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="currentColor">${PATHS[name] ?? ''}</svg>`;
  }
}

let registered = false;

/** Register the <bs-icon> custom element once. Safe to call repeatedly. */
export function registerIcons(): void {
  if (registered) return;
  if (typeof customElements === 'undefined') return; // headless/unit-test environments without a DOM custom-element registry
  if (!customElements.get('bs-icon')) customElements.define('bs-icon', BsIcon);
  registered = true;
}

/** Names available to <bs-icon name="…">. Exported for tests/tooling. */
export const ICON_NAMES: readonly string[] = Object.keys(PATHS);

/** Create a <bs-icon> element directly (typed helper, avoids raw createElement callsites). */
export function iconEl(name: IconName, size = 16, opacity?: number): HTMLElement {
  const el = document.createElement('bs-icon');
  el.setAttribute('name', String(name));
  el.setAttribute('size', String(size));
  if (opacity !== undefined) el.setAttribute('op', String(opacity));
  return el;
}
