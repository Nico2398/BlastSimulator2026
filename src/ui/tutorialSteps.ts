...
function gs(state: GameState): Record<string, unknown> {
  return state as unknown as Record<string, unknown>;
}

/** Safe access to employees array from mock-friendly state. */
function getEmployees(state: GameState): { role: string; hunger: number; fatigue: number; breakNeed: number; id: number }[] {
  const e = gs(state).employees;
...
function getVehicles(state: GameState): { driverId: number | null }[] {
  const v = gs(state).vehicles;
...
function getBuildings(state: GameState): { type: string }[] {
  const b = gs(state).buildings;
...
function countBuildingsOfType(state: GameState, buildingType: string): number {
  return getBuildings(state).filter(b => b.type === buildingType).length;
}

/** Count vehicles with a driver assigned. */
function countVehiclesWithDriver(state: GameState): number {
